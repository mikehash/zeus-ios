...
struct CommissioningView: View {
    ...
    ///
    /// The picker itself now lives in `RoutePicker.swift`, extracted so the
    /// NODES sheet can present the same surface after onboarding. It moved
    /// rather than being copied because every census that keeps it honest is
    /// file-scoped, and a second copy would have inherited none of them.
    ///
    /// WHAT STAYED HERE is the only part that was ever about commissioning:
    /// the record write and the step advance. `RoutePicker` hands the three
    /// chosen values up; this closure is what makes them a commission.
    private var routesStep: some View {
        RoutePicker(keys: keys, ctaTitle: "VALIDATE + CONTINUE") { id, model, baseURL in
...
/// Shared with `RoutePicker` for the same reason as `RouteCard` above.
...
/// Shared with `RoutePicker`, which is why it is no longer `private`: the
/// picker moved out of this file and the card went with its call site.
